...
## Transformation log base e des valeurs individuelles du tableau
...
# exemple sur la 3eme métabolite du tableau
...
g <-
  ...
  labs(title = varname, x = "temperature conditions", y = "metabolite abundance") + # pour définir tous les titres du graphique
...
pdf("boxplot metabolome 135 métabolites.pdf")
...
write.table(
  table_selection,
  file = "metabolome_selection_70.csv",
  append = F,
  sep = ";",
  dec = ".",
  row.names = T,
...
* on sélectionne ainsi 70 métabolites
...
pdf("boxplot Tukey metabolome 135 métabolites.pdf")
...
```{r}
## exemple d'un profil de variation sur la température : "a-b-b"

matrice.tuk.ftr<-matrice.tuk[,which(matrice.tuk[1,]=="a" & matrice.tuk[2,]=="b" & matrice.tuk[3,]=="b")]
...
matrice.tuk.ftr <-
  ...
  select(metabolites, everything()) %>% filter(Low == "a", Medium == "b", Elevated == "b")
...
write.table(t(table3),"tableau final anova metabolome.csv",dec=".",sep=";",append=F,row.names=T,col.names=NA)
